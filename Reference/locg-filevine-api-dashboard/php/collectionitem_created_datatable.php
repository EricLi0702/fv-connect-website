...
if($searchValue != ''){
   $searchQuery .= " and (demand_type like '%".$searchValue."%' or 
      project_id like '%".$searchValue."%' or 
      webhook_url like'%".$searchValue."%' ) ";
}

## Total number of records without filtering
$sel = mysqli_query($con,"select count(*) as allcount from collectionitem_created");
$records = mysqli_fetch_assoc($sel);
$totalRecords = $records['allcount'];

## Total number of records with filtering
$sel = mysqli_query($con,"select count(*) as allcount from collectionitem_created WHERE 1 ".$searchQuery);
$records = mysqli_fetch_assoc($sel);
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$query = "select * from collectionitem_created WHERE 1 ".$searchQuery." order by ".$columnName." ".$columnSortOrder." limit ".$row.",".$rowperpage;
...
while($row = $statement->fetch_object()) {
    ...
    $sub_array['demand_type'] = $row->demand_type;
